...
- **VS Code extension tests (follow-ups):** assert **scroll alignment** / visible ranges in the Extension Host; cover **Angles** (picker + paths) with a dedicated fixture; deepen preview assertions beyond smoke. Runner: `bash scripts/test-vscode-extension.sh` ([`packages/vscode/.vscode-test.mjs`](../../packages/vscode/.vscode-test.mjs)).
...
2. **VS Code**: (a) **Synchronized scroll** — tighten bidirectional stability, block-aware jumps with large `index.json`, long files, wrapped editors, and edge cases around `revealRange` / debounced rebuilds (`packages/vscode/src/extension.ts`, `packages/core/src/scroll-sync.ts`). (b) **Extension Host tests** — extend `@vscode/test-cli` coverage with scroll-range / block alignment checks and **Angles** UX (fixture + scripted quick picks); keep **separate** from **Cypress** static-site E2E (`cypress/e2e/`, **`e2e-static`** / `e2e-publish-checks.yml`). (c) Webview preview parity with `@commentray/render` output and richer block gutter UX.
...
| **Local static preview + reload**       | No automatic **browser** livereload on rebuild (refresh manually, or add browser-sync / dev middleware later). Watcher does not include `packages/render` sources — change render code → run **`npm run build`** (or rely on **`scripts/serve.sh`**’s initial package builds) before edits take effect. |
| **VS Code**                             | **Scroll sync:** polish feel, edge cases, performance on large buffers. **Extension Host tests:** add scroll / visible-range assertions and **Angles** coverage (see §Testing matrix — VS Code extension tests). |
...
| VS Code extension tests (Electron; Linux CI uses xvfb)            | `bash scripts/test-vscode-extension.sh` or `npm run test:vscode-extension`               |
...
- **VS Code** — Scroll-sync polish; extend `@vscode/test-cli` suite with alignment / **Angles** coverage (see §Testing matrix and gaps table).
