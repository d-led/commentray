...
- **Local Pages preview (watch):** **`npm run serve`** and **`npm run pages:serve`** both run [`scripts/serve.sh`](../scripts/serve.sh) (builds the CLI stack once, then **`commentray serve`**): rebuilds `_site` when `.commentray.toml`, static-site inputs, companions under `.commentray/`, or `index.json` change, and serves over HTTP (default **4173**; override with `npm run serve -- --port 8080`). Refresh the browser after rebuilds; there is no built-in livereload yet.
