...
- **Local Pages preview (watch):** **`npm run serve`** and **`npm run pages:serve`** both run [`scripts/serve.sh`](../scripts/serve.sh), which builds the CLI stack then runs [`scripts/serve-with-package-watch.mjs`](../scripts/serve-with-package-watch.mjs): that layer watches `packages/{core,render,code-commentray-static,cli}/src` (and render's `esbuild-code-browser-client.mjs`), rebuilds affected workspace packages on save, and restarts **`commentray serve`** so Node reloads `dist/`. Inside the CLI, **`commentray serve`** still rebuilds `_site` when `.commentray.toml`, static-site inputs, companions under `.commentray/`, or `index.json` change, and serves over HTTP (default **4173**; override with `npm run serve -- --port 8080`). Set **`COMMENTRAY_SERVE_NO_PACKAGE_WATCH=1`** to skip the workspace watcher (one-shot package builds only). Refresh the browser after rebuilds; there is no built-in livereload yet. For native file watching on the workspace tree, set **`COMMENTRAY_SERVE_PACKAGE_WATCH_POLL=0`** (defaults to polling to avoid **EMFILE** with small `ulimit -n`).
