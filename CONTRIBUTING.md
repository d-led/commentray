...
- **Before review, run:** `npm run quality:gate` — same script path as the **`ci.yml`** **`quick`** job (format, ESLint, shellcheck, dupes, typecheck, unit tests). Cypress + static site runs as **`e2e-static`** after that; see README **GitHub Actions**.
